...
type
  ...
  published
    ...
    property SplitChar: AnsiChar read FSplitChar write FSplitChar default #10;
...
begin
  ...
  begin
    ...
    repeat
      TastyStrPos := {Pos}AnsiStrings.AnsiPos(#13#10, OutputBuffer);
...
procedure TConsoleIO.ReceiveOutput(Buf: Pointer; Size: Integer);
  ...
  begin
    iPos:= Pos(AnsiString(']:'), OutputBuffer);
...
begin
  ...
  *)
  if (Pos(AnsiString(#13#10), OutputBuffer) > 0) then SplitMode:= sm0D0A
  ...
  if not SplitReceive or
    ...
    repeat
      TinnRCheck_2;
      TastyStrPos:= Pos(AnsiString(#13#10),
                        OutputBuffer);
      if (TastyStrPos = 0) then Break;
      FOnReceiveOutput(Self,
                       Copy(OutputBuffer,
...
begin
  ...
  Move(Buf^,
       Cmd[1],
       Size);
  if (Pos(AnsiString(#13#10), Cmd) > 0) then sSplit:= #13#10
                                        else sSplit:= #10;
  repeat
  ...
  repeat
    if not ReadFile(Handle,
      ...
      try
        ...
        on E: Exception do
        begin
          Err:= AnsiString(E.Message);
          Windows.SendMessage(FWindowHandle,
                              MIO_ERROR,
                              Integer(PAnsiChar(Err)),
                              Length(Err) + 1);
        end;
      end;
